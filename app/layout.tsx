...
function LayoutContent({ children }: { children: React.ReactNode }) {
  ...
  // Determine if we are on a login or onboarding screen
    ...
    <body className={`${inter.className} transition-colors duration-300 min-h-screen bg-[#f5f5f7] dark:bg-[#000000] text-[#1d1d1f] dark:text-[#f5f5f7]`}>
      ...
      <nav className="sticky top-0 z-40 backdrop-blur-md bg-white/70 dark:bg-[#161617]/70 border-b border-[#e8e8ed] dark:border-neutral-900 px-8 py-3 flex items-center justify-between transition-colors duration-300">
        <div className="flex items-center gap-8">
          ...
          {/* Render navigation links ONLY if user is authenticated and onboarding is finished */}
          {user && onboardingComplete && !isAuthScreen && (
            <div className="flex items-center gap-6">
              <a
                href="/"
                className={`apple-indicator-glow-sweep apple-dock-indicator apple-nav-border apple-zoom-indicator apple-tab-slide apple-indicator-sweep ${pathname === '/' ? 'active text-[#1d1d1f] dark:text-[#f5f5f7]' : 'text-neutral-500 hover:text-[#1d1d1f] dark:text-neutral-400 dark:hover:text-neutral-200'} text-xs font-medium transition-colors`}
              >
                Dashboard
              </a>
              <a
              ...
              <a
                href="/analytics"
                className={`apple-indicator-glow-sweep apple-dock-indicator apple-nav-border apple-zoom-indicator apple-tab-slide apple-indicator-sweep ${pathname === '/analytics' ? 'active text-[#1d1d1f] dark:text-[#f5f5f7]' : 'text-neutral-500 hover:text-[#1d1d1f] dark:text-neutral-400 dark:hover:text-neutral-200'} text-xs font-medium transition-colors`}
              >
                Analytics
              </a>
              <a
                href="/settings"
                className={`apple-indicator-glow-sweep apple-dock-indicator apple-nav-border apple-zoom-indicator apple-tab-slide apple-indicator-sweep ${pathname === '/settings' ? 'active text-[#1d1d1f] dark:text-[#f5f5f7]' : 'text-neutral-500 hover:text-[#1d1d1f] dark:text-neutral-400 dark:hover:text-neutral-200'} text-xs font-medium transition-colors`}
              >
                Settings
              </a>
            </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Render premium Sign Out button if user is logged in */}
          {user && (
            <button
              onClick={logout}
